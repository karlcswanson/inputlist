...
var output = fs.createWriteStream("./public/scans/wwb.csv");
...
function rtlScan(){
  var args = util.format("-f %s:%s:%s -1 -",rtl_config["lower_frequency"],rtl_config["upper_frequency"],rtl_config["bin_size"]);
  ...
  exec(command, function(error, stdout, stderr){
    ...
    lineReader.on('close',function(){
      ...
      fs.writeFileSync('./public/scans/sdr.json', JSON.stringify( datab ) , 'utf-8');
    });
  });
}
